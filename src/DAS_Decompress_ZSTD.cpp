                     ...
                     const hsize_t* dims)
{
    ...
    // write data to file
    int status = H5Dwrite(output_dataset, H5T_NATIVE_SHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, input.data());    // type of data in buffer (int16)
               ...
               std::string DATASET_NAME, int N_ROWS, int N_COLS)
{
    ...
    size_t cSize = nx_comp_input * sizeof(uint8_t);

    // create and fill uint8 array that contains the 1D compressed data
    std::vector<uint8_t> comp_input = read_data_uint8(file_path, DATASET_NAME, 
                                                       comp_input_dims, 1);
    
    void* const cBuff = (void* const) comp_input.data();
    ...
    void* const rBuff = (void* const)malloc((size_t)rSize);
    ...
    std::cout << "Decompressed Size: " << dSize << "\n";

    // read metadata from compressed data and initialize decompression parameters
    size_t decomp_size = (int) (dSize / sizeof(int16_t));
    std::vector<int16_t> decomp_output_untransposed(decomp_size);
    int16_t* rBuff_ = (int16_t*) rBuff;
    for (int i = 0; i < decomp_size; i++)
    {
	decomp_output_untransposed[i] = rBuff_[i];
    }
    int16_t TRANSPOSE = decomp_output_untransposed[0];
    decomp_output_untransposed.erase(decomp_output_untransposed.begin(), decomp_output_untransposed.begin() + 1);
    std::cout << "Decompressed Output Size: " << decomp_output_untransposed.size() << "\n";

    // transform data back to original uncompressed data
    std::vector<int16_t> decomp_output;
    ...
    if (TRANSPOSE == 0)
    {
        decomp_output = decomp_output_untransposed;
    }
    else
    {
        transpose_start_time = GetTimeMs64();
        transpose_data(decomp_output_untransposed, decomp_output, N_COLS,
                        N_ROWS);      
