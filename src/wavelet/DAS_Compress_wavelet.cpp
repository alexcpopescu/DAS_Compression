...
int write_data_uint32(std::vector<uint32_t> input, std::string file_name, std::string dataset_name, 
                     const hsize_t* dims)
{
    ...
    hid_t datatype = H5Tcopy(H5T_NATIVE_UINT);                                                     // type of data on disk (int16)
    ...
    // write data to file
    int status = H5Dwrite(output_dataset, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, input.data());    // type of data in buffer (int32)
...
float_t min(std::vector<float_t> arr) 
{
    float_t min_val = 0;

    for (int i=0; i<arr.size(); i++)
    {
        float_t curr_val = arr[i];
        if (curr_val < min_val)
        {
            min_val = curr_val;
        }
...
float_t max(std::vector<float_t> arr) 
{
    float_t max_val = 0;
...
double mean(std::vector<float_t> arr) 
{
    double sum = 0;
...
double absolute_mean(std::vector<float_t> arr) 
{
    double sum = 0;
...
double std_dev(std::vector<float_t> arr)
{
    double std = 0;
    double avg = mean(arr);
                     ...
                     std::string DATASET_NAME, bool TRANSPOSE)
{
    ...
    std::vector<int16_t> uncomp_input = read_data_int16(file_path, DATASET_NAME, 
                                                               uncomp_input_dims, 2);
    
    // split input into two arrays in order to work around TiLib's max dimensions
    int input_size = nx_uncomp_input*ny_uncomp_input;
    std::vector<int16_t> uncomp_input1(input_size/2);
    //std::vector<int16_t> uncomp_input2(input_size/2);
    for (int n = 0; n < input_size / 2; n++)
    {
	    int i = n / nx_uncomp_input;
	    int j = n % (ny_uncomp_input / 2);
        uncomp_input1[n] = uncomp_input[nx_uncomp_input * i + j];
        //uncomp_input2[n] = uncomp_input[nx_uncomp_input * i + ny_uncomp_input / 2 + j];
    }
    ...
    // Allocate buffer to store DAS data as image data
    unsigned char *in_buf1 = (unsigned char *) malloc(uncomp_input1.size()*3);
    //unsigned char *in_buf2 = (unsigned char *) malloc(uncomp_input2.size()*3);
    unsigned char *ptr1 = in_buf1;
    //unsigned char *ptr2 = in_buf2;
    
    // Iterate through the inputs and split each value across 3 RGB channels
    for (int i = 0; i <= uncomp_input1.size(); i++)
    {
        ptr1[0] = (uncomp_input1[i] >> 8) & 0xF;
        //ptr2[0] = (uncomp_input2[i] >> 8) & 0xF;
        ptr1[1] = (uncomp_input1[i] >> 4) & 0xF;
        //ptr2[1] = (uncomp_input2[i] >> 4) & 0xF;
        ptr1[2] = uncomp_input1[i] & 0xF;
        //ptr2[2] = uncomp_input2[i] & 0xF;
        ptr1 += 3;
       // ptr2 += 3;
    }

    unsigned char *out_buf1 = (unsigned char *) malloc(uncomp_input1.size()*3);
    //unsigned char *out_buf2 = (unsigned char *) malloc(uncomp_input2.size()*3);
    int actual_size1;
    //int actual_size2;

    int result1 = TiCompress(in_buf1, out_buf1, nx_uncomp_input, ny_uncomp_input / 2, DAUB97, TRUECOLOR, uncomp_input1.size(), &actual_size1, 90, 5, 5, 5);
    //int result2 = TiCompress(in_buf2, out_buf2, nx_uncomp_input, ny_uncomp_input / 2, DAUB97, TRUECOLOR, uncomp_input2.size(), &actual_size2, 90, 5, 5, 5);
    ...
    std::cout << "Size of compressed data 1: " << actual_size1 << "\n\n"; 
    //std::cout << "Size of compressed data 2: " << actual_size2 << "\n\n"; 
    std::cout << result1 << "\n\n"; 
    //std::cout << result2 << "\n\n"; 
    ...
    std::vector<uint8_t> comp_output(output_size);
    hsize_t comp_dims [1] = {output_size};
    for (int i = 0; i < output_size; i++)
    {
	    comp_output[i] = comp_output_ptr[i];
    }
    ...
    double comp_ratio = ((double) (nx_uncomp_input * ny_uncomp_input * sizeof(int16_t))) 
                        / ((double) (output_size * sizeof(uint8_t)));
