...
std::vector<double> read_data_int16(std::string file_name, std::string dataset_name, int* dims, int rank)
{
    // open the file and dataset using default params
    hid_t file = H5Fopen(file_name.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    hid_t dataset = H5Dopen(file, dataset_name.c_str(), H5P_DEFAULT);
    ...
    // read in the data and store it to the array
    int status = H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data.data());
    assert(status >= 0);                                                                            // ensure the read executed successfully
...
int write_data_uint8(void const const* input, std::string file_name, std::string dataset_name, 
                     const hsize_t* dims)
{
    // create new file to store compressed data
    hid_t output_file = H5Fcreate(file_name.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    ...
    // write data to file
    int status = H5Dwrite(output_dataset, H5T_NATIVE_UCHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, input);
    assert(status >= 0);                                                                            // ensure the write executed successfully
...
void transpose_data(std::vector<double> &input, std::vector<double> &output, const int N, const int M)
{
    for (int n = 0; n < N * M; n++)
    {
	    int i = n / N;
...
double absolute_mean(std::vector<double> arr) 
{
    double sum = 0;
                     ...
                     std::string DATASET_NAME, bool TRANSPOSE)
{
    ...
    std::vector<double> uncomp_input_untransposed = read_data_int16(file_path, DATASET_NAME, 
                                                               uncomp_input_dims, 2);
    uint64_t read_end_time = GetTimeMs64();
    std::cout << "Read Time: " << (read_end_time - read_start_time) << "ms\n\n";
    std::vector<double> uncomp_input;
    ...
    else
    {
    	...
    	std::vector<double> uncomp_input_transposed(uncomp_input_untransposed.size());
    ...
    */ 
    if (!TRANSPOSE)
    {
        uncomp_input.insert(uncomp_input.begin(), 0.0);
    }
    else
    {
        uncomp_input.insert(uncomp_input.begin(), 1.0);
    }
    ...
    const std::array<std::size_t, 1> shape = {nx_uncomp_input*ny_uncomp_input+1};
    const mgard::TensorMeshHierarchy<1, double> hierarchy(shape);
    const size_t ndof = hierarchy.ndof();
    std::cout << "ndof = " << ndof << "\n";
    const mgard::CompressedDataset<1, double> compressed = mgard::compress(hierarchy, uncomp_input.data(), s, tol);

    void const *const comp_output = compressed.data();  
    ...
    hsize_t comp_dims [1] = {compressed.size()};
    for (int i = 0; i < 10; i++)
    {
	std::cout << std::hex << ((uint16_t)((uint8_t*) comp_output)[i]) << "\n";
    }    
    ...
    double min_val_uncomp = min(uncomp_input_untransposed);
    double max_val_uncomp = max(uncomp_input_untransposed);
